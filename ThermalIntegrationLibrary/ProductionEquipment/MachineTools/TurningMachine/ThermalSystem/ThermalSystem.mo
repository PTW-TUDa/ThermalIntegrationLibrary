within ThermalIntegrationLibrary.ProductionEquipment.MachineTools.TurningMachine.ThermalSystem;
model ThermalSystem
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a CoolingLubricant "Thermal port for cooling system of machine tool" annotation (Placement(transformation(extent={{190,-10},{210,10}})));
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor thermalConductor(G=89) annotation (Placement(transformation(extent={{90,-10},{110,10}})));
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor thermalConductor2(G=38.14) annotation (Placement(transformation(extent={{90,90},{110,110}})));
  ...
  Modelica.Blocks.Interfaces.RealOutput T_machine annotation (Placement(transformation(
        ...
        rotation=90,
        origin={0,210})));
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a MachineCooling annotation (Placement(transformation(extent={{192,90},{212,110}})));
equation
  connect(thermalConductor1.port_b, hall) annotation (Line(points={{110,-100},{200,-100}}, color={191,0,0}));
  connect(thermalConductor.port_a, heatCapacitor.port) annotation (Line(points={{90,0},{0,0}},           color={191,0,0}));
  connect(thermalConductor.port_b, CoolingLubricant) annotation (Line(points={{110,0},{200,0}},     color={191,0,0}));
  ...
  connect(thermalConductor2.port_b, MachineCooling) annotation (Line(points={{110,100},{202,100}},
                                                                                               color={191,0,0}));
  connect(thermalConductor1.port_a, heatCapacitor.port) annotation (Line(points={{90,-100},{0,-100},{0,0}}, color={191,0,0}));
  connect(thermalConductor2.port_a, heatCapacitor.port) annotation (Line(points={{90,100},{0,100},{0,0}}, color={191,0,0}));
