...
equation
/************************* WeightingFactors ********************************/
  weightingFactor[1] = ThermalIntegrationLib.Internals.smoothTransition(
    time,
    startTime + firstStepPeriod/2.0,
    firstStepPeriod);
  weightingFactor[2] = ThermalIntegrationLib.Internals.smoothTransition(
    time,
    startTime + firstStepPeriod + stepPeriod/2.0,
    stepPeriod);
  weightingFactor[3] = ThermalIntegrationLib.Internals.smoothTransition(
    time,
    startTime + firstStepPeriod + stepPeriod + stepPeriod/2.0,
    stepPeriod);

  for i in 4:auxPoints loop
    weightingFactor[i] = ThermalIntegrationLib.Internals.smoothTransition(
      time,
