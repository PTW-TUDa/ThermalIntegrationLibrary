...
model FactoryBuilding_productionHallRimOffices_eff "A production hall with rim offices"
  ...
  ThermalZones.ProductionHall productionHall(
    ...
    Tmax_occ=299.15,
    hasEnclosure=true,
    enclosureVolume=enclosure.roo.V,
    surBou(til={Buildings.Types.Tilt.Wall,Buildings.Types.Tilt.Wall,Buildings.Types.Tilt.Wall,Buildings.Types.Tilt.Wall,Buildings.Types.Tilt.Ceiling}, A={enclosure.length*enclosure.zoneHeight,enclosure.length*enclosure.zoneHeight,enclosure.width*enclosure.zoneHeight,enclosure.width*enclosure.zoneHeight,enclosure.roo.AFlo}),
    nConExt=5,
    nConExtWin=4,
    nConBou=5,
    nSurBou=5,
    ...
    datConBou(
      ...
      A={productionHall.roo.AFlo - enclosure.roo.AFlo,office1_EG.length*office1_EG.zoneHeight,office1_OG.length*office1_OG.zoneHeight,office2_EG.length*office2_EG.zoneHeight,office2_OG.length*office2_OG.zoneHeight},
      each stateAtSurface_a=false),
    ...
    lastDayOfWorkWeek=Types.Weekday.Friday,
    schedules=[0,0,0.05,0,0.05; 6,0.1,0.1,0.05,0.05; 7,0.2,0.3,0.05,0.05; 8,0.95,0.3,0.05,0.05; 9,0.95,0.9,0.05,0.05; 13,0.95,0.5,0.05,0.05; 17,0.3,0.5,0.05,0.05; 18,0.1,0.3,0,0.05; 19,0.1,0.3,0,0.05; 20,0.1,0.2,0,0.05; 22,0,0.1,0,0.05; 23,0,0.05,0,0.05; 24,0,0.05,0,0.05],
    nFPorts=1)                                                                                                                                                                                                         annotation (Placement(transformation(extent={{-22,-24},{10,8}})));
  ...
  InternalGains.Gain_th_per_area machineDummy(shareRad=0.3, nominalPower=(61*10000)/productionHall.roo.AFlo)
  ...
  ThermalZones.Enclosure enclosure(
    ...
    pipeTABS=datPip,
    nCirTABS=400,
    iLayPipTABS=enclosureWalls.nLay - 1,
    lengthTABS=enclosure.width,
    m_flow_nominalTABS=0.4*1000/3600,
    nConBou=5,
    nSurBou=1,
    nConTABS=1,
    datConBou(
      ...
      til={Buildings.Types.Tilt.Floor,Buildings.Types.Tilt.Wall,Buildings.Types.Tilt.Wall,Buildings.Types.Tilt.Wall,Buildings.Types.Tilt.Wall},
      azi={0,Buildings.Types.Azimuth.N,Buildings.Types.Azimuth.S,Buildings.Types.Azimuth.W,Buildings.Types.Azimuth.E},
      A={enclosure.roo.AFlo,enclosure.length*enclosure.zoneHeight,enclosure.length*enclosure.zoneHeight,enclosure.width*enclosure.zoneHeight,enclosure.width*enclosure.zoneHeight},
      each stateAtSurface_a=false),
    surBou(til={Buildings.Types.Tilt.Ceiling}, A={enclosure.roo.AFlo}),
    layersTABS=enclosureWalls,
    nFPorts=1)  annotation (Placement(transformation(extent={{32,-26},{52,-6}})));
  InternalGains.Gain_th_per_area machineDummy1(shareRad=0.3, nominalPower=(10*20000)/productionHall.roo.AFlo)
                                                                                                  annotation (Placement(transformation(extent={{-70,-28},{-62,-20}})));
  parameter HeatTransfer.Data.OpaqueConstructions.Ducon_VacuumInsulated enclosureWalls annotation (Placement(transformation(extent={{84,-32},{94,-22}})));
  Buildings.Fluid.Sources.MassFlowSource_T boundary(
    redeclare package Medium = Buildings.Media.Water,
    m_flow=0.4*1000/3600,
    T=291.15,
    nPorts=1) annotation (Placement(transformation(extent={{-76,-94},{-60,-78}})));
  Buildings.Fluid.Sources.Boundary_pT bou(redeclare package Medium = Buildings.Media.Water, nPorts=1) annotation (Placement(transformation(extent={{72,-48},{62,-38}})));
  parameter HeatTransfer.Data.Pipes.PEX_DN_3dot35                                                  datPip annotation (Placement(transformation(extent={{72,-32},{82,-22}})));
equation
  ...
  connect(machineDummy.Qth_conv,productionHall. Q_gainConv) annotation (Line(points={{-61.28,-15.2},{-43.64,-15.2},{-43.64,-21.28},{-24.4,-21.28}},
                                                                                                                                                  color={0,0,127}));
  connect(productionHall.insideRoomTemperature, enclosure.infiltrationTemperature) annotation (Line(points={{11.92,2.24},{46,2.24},{46,-4.4}}, color={0,0,127}));
  connect(productionHall.insideRoomTemperature, enclosure.ventilationTemperature) annotation (Line(points={{11.92,2.24},{42,2.24},{42,-4.4}}, color={0,0,127}));
  connect(schedTable.y[1], machineDummy1.schedule) annotation (Line(points={{-79.3,-13},{-74,-13},{-74,-24},{-70.8,-24}}, color={0,0,127}));
  connect(machineDummy1.Qth_rad, enclosure.Q_gainRad) annotation (Line(points={{-61.28,-26.8},{-34,-26.8},{-34,-32},{24,-32},{24,-21.3},{30.5,-21.3}}, color={0,0,127}));
  connect(machineDummy1.Qth_conv, enclosure.Q_gainConv) annotation (Line(points={{-61.28,-25.2},{-15.64,-25.2},{-15.64,-24.3},{30.5,-24.3}}, color={0,0,127}));
  connect(GroundTemperature.y, enclosure.T_ground) annotation (Line(points={{-85.5,-61},{-80,-61},{-80,-30},{34,-30},{34,-32},{40.9,-32},{40.9,-27.5}}, color={0,0,127}));
  connect(enclosure.surf_conBou[:], productionHall.surf_surBou[1:4]) annotation (Line(points={{50.2,-27},{50.2,-36},{-0.56,-36},{-0.56,-24.96}},color={191,0,0}));
  connect(enclosure.surf_conTABS[1], productionHall.surf_surBou[5]) annotation (Line(points={{47.8,-27},{46,-27},{46,-36},{-0.56,-36},{-0.56,-24.32}},color={191,0,0}));
  connect(boundary.ports[1:1], enclosure.portTABS_a) annotation (Line(points={{-60,-86},{-14,-86},{-14,-17.4},{31,-17.4}}, color={0,127,255}));
  connect(bou.ports[1:1], enclosure.portTABS_b) annotation (Line(points={{62,-43},{46,-43},{46,-34},{34.6,-34},{34.6,-27}}, color={0,127,255}));
  connect(productionHall.fPorts[1], enclosure.fPorts[1]) annotation (Line(points={{-23.6,0},{-26,0},{-26,10},{22,10},{22,-11},{31,-11}}, color={0,127,255}));
 annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(coordinateSystem(
          preserveAspectRatio=false)),
    Documentation(info="<html>
</html>"));
end FactoryBuilding_productionHallRimOffices_eff;
