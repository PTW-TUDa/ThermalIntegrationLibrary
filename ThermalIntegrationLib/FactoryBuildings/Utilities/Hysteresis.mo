...
block Hysteresis "Transform Real to Boolean signal with Hysteresis"
  ...
  Modelica.Blocks.Interfaces.RealInput u annotation (Placement(transformation(extent={{-140,0},{-100,40}})));
  Modelica.Blocks.Interfaces.BooleanOutput y annotation (Placement(transformation(extent={{100,-10},{120,10}})));

  Modelica.Blocks.Interfaces.RealInput uMin "if y=true and u<=uMin, switch to y=false" annotation (Placement(transformation(extent={{-140,-96},{-100,-56}})));
  Modelica.Blocks.Interfaces.RealInput uMax "if y=false and u>=uMax, switch to y=true" annotation (Placement(transformation(extent={{-140,-64},{-100,-24}})));
initial equation
  pre(y) = pre_y_start;
equation
  assert(uMax > uMin,"Hysteresis limits wrong (uMax <= uMin)");
  y = not pre(y) and u > uMax or pre(y) and u >= uMin;
  annotation (
