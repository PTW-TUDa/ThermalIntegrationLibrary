...
equation
/************************* WeightingFactors ********************************/
  weightingFactor[1] = ThermalIntegrationLib.BaseClasses.Internals.smoothTransition(
      time,
      startTime + firstStepPeriod/2.0,
      firstStepPeriod);
  weightingFactor[2] = ThermalIntegrationLib.BaseClasses.Internals.smoothTransition(
      time,
      startTime + firstStepPeriod + stepPeriod/2.0,
      stepPeriod);
  weightingFactor[3] = ThermalIntegrationLib.BaseClasses.Internals.smoothTransition(
      time,
      startTime + firstStepPeriod + stepPeriod + stepPeriod/2.0,
      stepPeriod);

  for i in 4:auxPoints loop
    weightingFactor[i] = ThermalIntegrationLib.BaseClasses.Internals.smoothTransition(
        time,
