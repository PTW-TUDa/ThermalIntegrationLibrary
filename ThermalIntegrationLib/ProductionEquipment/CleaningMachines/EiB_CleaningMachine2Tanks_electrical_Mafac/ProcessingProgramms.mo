...
package ProcessingProgramms
  ...
  algorithm
     ...
     for j in 1:rows loop
       ...
       elseif stateTable[j,1] >= 8 and stateTable[j,1] <= 10 then // Add time for cleaning with MT2
  ...
  equation
    ...
    if stateTable[i,1] >= 5 and stateTable[i,1] <= 7 and pre(ready) then // if cleaning with MT1 is running, set tank_time to remaining time in cleaning
      tank_time = dur_mt1 - (time-startTime) + 0.000001;
    elseif stateTable[i,1] >= 8 and stateTable[i,1] <= 10 then // if cleaning with MT1 is running, set tank_time to remaining time in cleaning
