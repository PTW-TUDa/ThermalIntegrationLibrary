...
equation
  ...
  if t1_state then // cleaning in progress, heat extraction by batch
    Q_dot_batch = m_batch*c_batch*(T_t1-T_batch)/tank_time;
  elseif t2_state then
    Q_dot_batch = m_batch*c_batch*(T_t2-T_batch)/tank_time;
  else // cleaning NOT in progress
    Q_dot_batch = 0;
  end if;

  der(T_batch) = Q_dot_batch/(m_batch*c_batch); // batch temperature slope
